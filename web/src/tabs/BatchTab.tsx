...
export default function BatchTab() {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['jobs', 'Batch'] });
  ...
  return (
    <Box>
      <Stack direction="row" justifyContent="space-between" alignItems="center" mb={3}>
        <Box>
          <Typography variant="h5" fontWeight={700}>Azure Batch Jobs</Typography>
          <Typography variant="body2" color="text.secondary">
            Heavy workloads fanned out across auto-scaling compute nodes
          </Typography>
        </Box>
      ...
      <RunHistoryDrawer
        ...
        onRerun={() => queryClient.invalidateQueries({ queryKey: ['jobs', 'Batch'] })}
      />
    </Box>
  );
}
